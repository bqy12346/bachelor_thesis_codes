...
class NCPNet(nn.Module):
    ...
    def __init__(self, n_classes: int, motor_neurons: int = 32, mixed_memory: bool = False):
        ...
        # E1: 去掉STFT,改用直接对原始信号的CNN前端(stride 4→5压缩时间)
...
class NCPClassifier:
    ...
    def fit(self, X_train, y_train, X_val, y_val):
        ...
        # 保存最优模型
        save_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "../output/ablation/E1_no_stft", self.task)
        os.makedirs(save_dir, exist_ok=True)
        ts       = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"cfc_ncp_no_stft_{ts}.pt"
        torch.save(best_state, os.path.join(save_dir, filename))
        print(f"Model saved to {save_dir}/{filename}")
        
        # ── [新增] 保存训练历史，供后续分析和曲线绘制 ──
        history_path = os.path.join(save_dir, f"cfc_ncp_no_stft_history_{ts}.npz")
        np.savez(
            history_path,
        ...
        # ── [新增] 自动画训练曲线 ──
        plot_path = os.path.join(save_dir, f"cfc_ncp_no_stft_curves_{ts}.png")
