...
class NCPNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # ── CfC-NCP + 时间注意力池化 ───────────────────────────────────────
        # E2 版本不使用注意力池化，直接取最后一个时间步的输出作为分类输入
...
class NCPClassifier:
    ...
    def fit(self, X_train, y_train, X_val, y_val):
        ...
        # 保存最优模型
        save_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "../output/ablation/E2_no_attn", self.task)
        os.makedirs(save_dir, exist_ok=True)
        ts       = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"cfc_ncp_no_attn_{ts}.pt"
        torch.save(best_state, os.path.join(save_dir, filename))
        print(f"Model saved to {save_dir}/{filename}")
        
        # ── [新增] 保存训练历史，供后续分析和曲线绘制 ──
        history_path = os.path.join(save_dir, f"cfc_ncp_no_attn_history_{ts}.npz")
        np.savez(
            history_path,
        ...
        # ── [新增] 自动画训练曲线 ──
        plot_path = os.path.join(save_dir, f"cfc_ncp_no_attn_curves_{ts}.png")
