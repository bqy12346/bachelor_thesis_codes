...
def draw_group(ax, edges, src_pos, dst_pos, color, top_k, dashed=False, alpha_override=None):
    if not edges:
        return
    ...
    for (si, di, w) in top:
        ...
        alpha = alpha_override if alpha_override is not None else (0.15 + 0.65 * w)
...
# ── 右图：Dense fully-connected wiring（对比用）─────────────────────────
ax2 = fig.add_subplot(1, 2, 2)
ax2.set_facecolor("#F8F8F8"); ax2.axis("off")
ax2.set_xlim(0, 1); ax2.set_ylim(0, 1)
ax2.set_title(
    f"Dense — fully connected (same neuron counts)\n"
    f"Sensory={SENSORY_N}  Inter={INTER_N}  Command={COMMAND_N}  Motor={MOTOR_N}",
    fontsize=11, pad=10,
)

# 全连接边（uniform weight=1）
all_SI = [(s, d, 1.0) for s in range(SENSORY_N) for d in range(INTER_N)]
all_IC = [(s, d, 1.0) for s in range(INTER_N)   for d in range(COMMAND_N)]
all_CM = [(s, d, 1.0) for s in range(COMMAND_N) for d in range(MOTOR_N)]

draw_group(ax2, all_SI, pos_s, pos_i, PAIR_COLORS["S→I"], len(all_SI), alpha_override=0.12)
draw_group(ax2, all_IC, pos_i, pos_c, PAIR_COLORS["I→C"], len(all_IC), alpha_override=0.18)
draw_group(ax2, all_CM, pos_c, pos_m, PAIR_COLORS["C→M"], len(all_CM), alpha_override=0.14)

draw_nodes(ax2, pos_s, np.ones(SENSORY_N), COLORS_NODE["sensory"])
draw_nodes(ax2, pos_i, np.ones(INTER_N),   COLORS_NODE["inter"])
draw_nodes(ax2, pos_c, np.ones(COMMAND_N), COLORS_NODE["command"])
draw_nodes(ax2, pos_m, np.ones(MOTOR_N),   COLORS_NODE["motor"])

for name, lx, n in [("Sensory", LAYER_X[0], SENSORY_N),
                     ("Inter",   LAYER_X[1], INTER_N),
                     ("Command", LAYER_X[2], COMMAND_N),
                     ("Motor",   LAYER_X[3], MOTOR_N)]:
    ax2.text(lx, 0.98, name,     ha="center", va="center",
             fontsize=10, fontweight="bold", color=COLORS_NODE[name.lower()])
    ax2.text(lx, 0.02, f"({n})", ha="center", va="center",
             fontsize=8, color=COLORS_NODE[name.lower()])

total_dense   = SENSORY_N*INTER_N + INTER_N*COMMAND_N + COMMAND_N*MOTOR_N
total_sparse  = len(SI) + len(IC) + len(CM) + len(CC)
ax2.text(0.5, 0.06,
         f"Total connections: {total_dense}  vs  NCP: {total_sparse}  "
         f"(sparsity {1-total_sparse/total_dense:.1%})",
