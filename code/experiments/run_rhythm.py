...
log_path = os.path.join("./output/", "exp_rhythm", "logs", f"exp_rhythm_{timestamp}.log")
...
try:
    ...
    # ── [统计新增] 保存 CSV ───────────────────────
    csv_path = os.path.join(OUTPUTFOLDER, "exp_rhythm", "results", "model_stats.csv")    
    save_results_csv(stat_records, csv_path)

    # ── [统计新增] 绘制对比图 ─────────────────────
    plot_dir = os.path.join(OUTPUTFOLDER, "exp_rhythm", "results", "plots")
