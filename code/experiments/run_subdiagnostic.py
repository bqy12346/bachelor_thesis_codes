...
log_path = os.path.join("./output/", "exp_subdiagnostic", "logs", f"exp_subdiagnostic_{timestamp}.log")
...
exp = SCP_Experiment(
    experiment_name    = "exp_subdiagnostic",
    task               = "subdiagnostic",
...
try:
    ...
    # ── [统计新增] 保存 CSV ───────────────────────
    csv_path = os.path.join(OUTPUTFOLDER, "exp_subdiagnostic", "results", "model_stats.csv")    
    save_results_csv(stat_records, csv_path)

    # ── [统计新增] 绘制对比图 ─────────────────────
    plot_dir = os.path.join(OUTPUTFOLDER, "exp_subdiagnostic", "results", "plots")
